...
void CloudShape::_updateBoundingBoxDim() {
  ...
  for (unsigned int j = 0; j < pts_.size() ; j++) {
      max_X = std::max<double>(max_X, pts_[j].x()) ;
      max_Y = std::max<double>(max_Y, pts_[j].y()) ;
      max_Z = std::max<double>(max_Z, pts_[j].z()) ;
      min_X = std::min<double>(min_X, pts_[j].x()) ;
      min_Y = std::min<double>(min_Y, pts_[j].y()) ;
      min_Z = std::min<double>(min_Z, pts_[j].z()) ;
  }
